...
#include "config.h"
#include "att.h"
#define SERVER_SUPPORTED_FEATURES 0x2B3A

#define Generic_Access 0X1800
#define Generic_Attribute 0x1801
#define Battery 0X180F
#define Heart_Rate 0X180D
#define Human_Interface_Device 0X1812
...
uint16_t gatts_handle_find_info_req(uint8_t *rx_buffer, uint16_t len, uint8_t *tx_buffer);
...
/////
#define GATT_UUID_SER_SUP_FEA 0X2B29
...
typedef struct
{
	void (*gatt_db_read)(uint16_t handle);
	void (*gatt_db_write)(uint16_t handle, uint8_t *value, uint8_t value_len);
...
extern gatt_server_manager_t gatt_server_manager;
